...
function getpercentage(x, y, elm) { 
    elm.find('.fab-fid-inner').html(y + '/' + x);
...
function pbmit_title_animation() {
	...
	ScrollTrigger.matchMedia({
		...
		var pbmit_var = jQuery('.fab-heading, .fab-heading-subheading');
		if (!pbmit_var.length) {
			return;
		}
		const quotes = document.querySelectorAll(".fab-heading-subheading .fab-title, .fab-heading .fab-title");
			...
			quotes.forEach(quote => {
				...
				var getclass = quote.closest('.fab-heading-subheading, .fab-heading').className;
...
function pbmit_extend_section() {
	const pbmit_elm = gsap.utils.toArray('.fab-extend-animation');
...
function pbmit_sticky_special() {	 
	...
	if (jQuery('.fab-sticky-col2-special').hasClass('.elementor-element-edit-mode')) {
		return;
	}
	ScrollTrigger.matchMedia({
		"(min-width: 1024px)": function() { 
			let pbmit_sticky_2 = jQuery(".fab-sticky-col2-special");
			let section = jQuery('.fab-sticky-special'); 
...
function pbmit_sticky() {
	...
	ScrollTrigger.matchMedia({
		"(min-width: 1200px)": function() {
			let pbmit_sticky_container = jQuery(".fab-sticky-col");
			let section = pbmit_sticky_container.closest('section');
			if (!section[0]) {
				section = pbmit_sticky_container.closest('.fab-sticky-section');
			} 
			let tl = gsap.timeline({
				scrollTrigger: {
...
function pbmit_img_animation() {
	const boxes = gsap.utils.toArray('.fab-animation-style1,.fab-animation-style2,.fab-animation-style3,.fab-animation-style4,.fab-animation-style5,.fab-animation-style6,.fab-animation-style7');
	boxes.forEach(img => {
		gsap.to(img, {
			scrollTrigger: {
				trigger: img,
...
function pbmit_staticbox_hover() {
	var pbmit_var = jQuery('.fab-element-static-box-style-1, .fab-element-service-style-5, .fab-element-service-style-7');
	if (!pbmit_var.length) {
		return;
	}
	pbmit_var.each(function() {
		var pbmit_Class = '.fab-element-posts-wrapper > .fab-ele-static-box, .swiper-hover-slide-nav .fab-hover-inner li, article.fab-service-style-7';
		jQuery(this)
		...
		jQuery(this)
			...
			.on('mouseover', function() {
				jQuery(this).addClass('fab-active').siblings().removeClass('fab-active');
			});
	});
}
...
var pbmit_hover_slide = function() {
	...
	if (typeof Swiper !== 'undefined') {
		...
		var pbmit_hover1 = new Swiper(".fab-hover-image", {
			grabCursor: true,
		...
		jQuery('.fab-main-hover-slider li').hover(function(e) {
			e.preventDefault();
...
function pbmit_tween_effect() {
	...
	const pbmit_tween = gsap.utils.toArray('.fab-tween-effect');
